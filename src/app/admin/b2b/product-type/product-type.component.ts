...
import { AfterViewInit, ChangeDetectorRef, Component, OnChanges, OnDestroy, SimpleChanges } from '@angular/core';
  ...
  onEditData(id:number = 0):void{
    ...
    if(id>0){
      ...
      this.serviceSub[2] = this.svc.load(id).subscribe({
        next: (data) =>{
          if ("name" in data){
            this.localObject = data as ProductType;
